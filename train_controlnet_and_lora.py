...
def log_validation(
    ...
    if len(args.validation_image) == len(args.validation_prompt):
        ...
        raise ValueError(
            "number of `args.validation_image` and `args.validation_prompt` should be"
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " that the checkpoint is better than the final trained model, the"
            " checkpoint can also be used for inference.Using a checkpoint for"
            " inference requires separate loading of the original pipeline and the"
            " individual checkpointed model components.See"
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Save more memory by using setting grads to None instead of zero. Be aware,"
            " that this changes certain"
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Proportion of image prompts to be replaced with empty strings. Defaults to"
            " 0 (no prompt replacement)."
        ),
    )
    parser.add_argument(
        "--validation_prompt",
        ...
        help=(
            ...
            " `--report_to`. Provide either a matching number of `--validation_image`s,"
            " a single `--validation_image` to be used with all prompts, or a single"
            " prompt that will be used with all `--validation_image`s."
        ),
    )
    parser.add_argument(
        "--validation_image",
        ...
        nargs="+",
        help=(
            "A set of paths to the controlnet conditioning image be evaluated every"
            " `--validation_steps` and logged to `--report_to`. Provide either a"
            " matching number of `--validation_prompt`s, a a single"
            " `--validation_prompt` to be used with all `--validation_image`s, or a"
            " single `--validation_image` that will be used with all"
            " `--validation_prompt`s."
        ),
    )
    parser.add_argument(
        ...
        default=4,
        help=(
            "Number of images to be generated for each `--validation_image`,"
    ...
    if args.resolution % 8 != 0:
        raise ValueError(
            "`--resolution` must be divisible by 8 for consistently sized encoded"
            " images between the VAE and the controlnet encoder."
        )
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    if args.caption_column is None:
        ...
        if caption_column not in column_names:
            raise ValueError(
                f"`--caption_column` value '{args.caption_column}' not found in dataset"
                f" columns. Dataset columns are: {', '.join(column_names)}"
            )
...
def main(args):
    ...
    low_precision_error_string = (
        ...
        " when starting training - even if doing mixed precision training, copy of the"
        " weights should still be float32."
    )

    if accelerator.unwrap_model(controlnet).dtype != torch.float32:
        raise ValueError(
            "Controlnet loaded as datatype"
            f" {accelerator.unwrap_model(controlnet).dtype}."
